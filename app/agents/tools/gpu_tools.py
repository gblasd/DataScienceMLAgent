...
import warnings
warnings.filterwarnings("ignore", message="X has a feature names", category=UserWarning)
warnings.filterwarnings("ignore", message='The pipeline instance is not fitted yet.', category=FutureWarning)
...
def build_preprocessing_pipeline(df, target: str):
    ...
    numeric_pipeline = Pipeline([
        ...
        ('float32', Float32Transformer(accept_parse=True)),
    ])
...
def split_regression_data(X, y, test_size=0.2, random_state=42):
    """Split data for regression (no stratificacion)"""
...
def train_classification_models(X_train, X_test, y_train, y_test, preprocessor):
    ...
    models = [
        ('Logisctic Regression', LogisticRegression(max_iter=500)),
    ...
    for name, model in models:
        ...
        f1 = f1_score(y_test, test_predictions, average='weiighted')
...
def train_regression_models(X_train, X_test, y_train, y_test, preprocessor, primary_metric='r2'):
    ...
    models = [
        ...
        ('Linear SVR', LinearSVR(max_iter=1000, tol=1e-3))
    ]
...
class Float32Transformer:
    """Picklable trasformer to convert arrays to float32 for memory efficiency."""

    def __init__(self, accept_sparse=False):
        self.accept_sparse = self.accept_sparse
